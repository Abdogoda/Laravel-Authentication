                    ...
                    <div class="flex items-center justify-between p-3 bg-gray-800 rounded-lg border border-gray-700 mb-4">
                        <div class="flex items-center">
                            @if ($session->agent['is_desktop'])
                                <i class="fas fa-desktop w-6 h-6 text-gray-200 mr-3"></i>
                            @else
                                <i class="fas fa-mobile-alt w-6 h-6 text-gray-200 mr-3"></i>
                            @endif
                            <div>
                                ...
                                <div class="text-sm text-gray-400">
                                    {{$session->ip_address}} @if ($session->is_current_device) <span class="text-green-500">This device</span> @endif
                                </div>
                                <div class="text-xs text-gray-500">{{$session->last_activity}}</div>
                            </div>
                        </div>
                        @if (!$session->is_current_device)
                            <form action="{{route('logout-device', $session)}}" method="post">
                                @csrf
                                <button type="submit" class="bg-red-600 text-white px-4 py-2 rounded-md">Log out</button>
                            </form>
                        @endif
                    </div>
                @endforeach
